...
public class Wheel : MonoBehaviour {
	...
	public float lateralTireStiffness; // lateral stiffness of the tire. The higher is the lateral stiffness, the lower is the lateral deflection of the tire in curves (tire is more stable). This influence force feedback too.
	...
	void Start(){
		...
		while (trs != null && trs.GetComponent<Rigidbody>() == null) trs = trs.parent;
		if (trs != null) body = trs.GetComponent<Rigidbody>();
	...
	public void SetTireStiffness(){
		...
		//else lateralTireStiffness=8*width*(pressure + 0.24f*optimalPressure)*(1 - (sidewallHeight/radius)*(radius/width))*1000;
		else lateralTireStiffness=pressure/(sidewallHeight*0.01f);
	...
	Vector3 RoadForce(float normalForce,  Vector3 groundNormal) {
		...
		if (cardynamics.physicMaterials.Length>0 && isDirty==false){ 
			int k=0;
			physicMaterial = hitDown.collider.sharedMaterial;
			if (physicMaterial!=null){
				for (i=0; i<cardynamics.physicMaterials.Length;i++){
					if (cardynamics.physicMaterials[i].physicMaterial==physicMaterial) {k=i;break;}
				}
				gripMaterial=cardynamics.physicMaterials[k].grip; 
				rollingFrictionCoefficient=cardynamics.physicMaterials[k].rollingFriction;
				staticFrictionCoefficient=cardynamics.physicMaterials[k].staticFriction;
				isSkidSmoke=cardynamics.physicMaterials[k].isSkidSmoke;
				isSkidMark=cardynamics.physicMaterials[k].isSkidMark;
				isDirty=cardynamics.physicMaterials[k].isDirty;
				surfaceType=cardynamics.physicMaterials[k].surfaceType;
			}
		}
...
/* 	float  BumpStopForce(float overtravel) {
	...
	void CalcSkidmarks(){
		if (slipVelo>slipVeloThreshold && isSkidMark==true){
