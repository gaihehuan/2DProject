...
public class Setup : MonoBehaviour {
	...
	void SavePhysicMaterialsData(){
		if (cardynamics.physicMaterials.Length>0){
			foreach(MyPhysicMaterial physicMaterial in cardynamics.physicMaterials){
				SaveValue("grip", physicMaterial.grip+"","physicMaterials-"+physicMaterial.surfaceType);
				SaveValue("rollingFriction", physicMaterial.rollingFriction+"","physicMaterials-"+physicMaterial.surfaceType);
				SaveValue("staticFriction", physicMaterial.staticFriction+"","physicMaterials-"+physicMaterial.surfaceType);
				SaveValue("isSkidSmoke", physicMaterial.isSkidSmoke+"","physicMaterials-"+physicMaterial.surfaceType);
				SaveValue("isSkidMark", physicMaterial.isSkidMark+"","physicMaterials-"+physicMaterial.surfaceType);
				SaveValue("isDirty", physicMaterial.isDirty+"","physicMaterials-"+physicMaterial.surfaceType);
			}
		}
	}	
	...
	void SaveArcaderData(){
		if (arcader!=null){
			SaveValue("enabled", arcader.enabled+"","arcader");
	...
	void LoadBodyData(){
		value=LoadValue("weight","body"); if (value!="") GetComponent<Rigidbody>().mass=float.Parse(value,System.Globalization.CultureInfo.InvariantCulture.NumberFormat);
	...
	void LoadPhysicMaterialsData(){
		if (cardynamics.physicMaterials.Length>0){
			foreach(MyPhysicMaterial physicMaterial in cardynamics.physicMaterials){
				value=LoadValue("grip", "physicMaterials-"+physicMaterial.surfaceType); if (value!="") physicMaterial.grip=float.Parse(value,System.Globalization.CultureInfo.InvariantCulture.NumberFormat);
				value=LoadValue("rollingFriction", "physicMaterials-"+physicMaterial.surfaceType); if (value!="") physicMaterial.rollingFriction=float.Parse(value,System.Globalization.CultureInfo.InvariantCulture.NumberFormat);
				value=LoadValue("staticFriction", "physicMaterials-"+physicMaterial.surfaceType); if (value!="") physicMaterial.staticFriction=float.Parse(value,System.Globalization.CultureInfo.InvariantCulture.NumberFormat);
				value=LoadValue("isSkidSmoke","physicMaterials-"+physicMaterial.surfaceType); if (value!="") physicMaterial.isSkidSmoke=bool.Parse(value);
				value=LoadValue("isSkidMark", "physicMaterials-"+physicMaterial.surfaceType); if (value!="") physicMaterial.isSkidMark=bool.Parse(value);
				value=LoadValue("isDirty", "physicMaterials-"+physicMaterial.surfaceType); if (value!="") physicMaterial.isDirty=bool.Parse(value);
			}
		}
	}		
	...
	void LoadArcaderData(){
		if (arcader!=null) {
			value=LoadValue("enabled", "arcader"); if (value!="") arcader.enabled=bool.Parse(value);
